// src/components/sections/fight-modal.tsx
'use client'
...
interface OpponentStats extends FighterStats {
  nombre: string
  record: string
  altura: string
  alcance: string
...
export function FightModal({ pelea, isOpen, onClose, isTopuria, fighterImage }: FightModalProps) {
  ...
  const currentRecord = isTopuria ? '17-0-0' : pelea.oponenteStats.record
  const currentNationality = isTopuria ? 'España/Georgia' : pelea.oponenteStats.nacionalidad
  const currentAge = isTopuria ? 27 : pelea.oponenteStats.edad
  const currentHeight = isTopuria ? '1.70m' : pelea.oponenteStats.altura
  const currentReach = isTopuria ? '68"' : pelea.oponenteStats.alcance
