...
export function FightSlider({ peleas, peleaSeleccionada, onSelectPelea }: FightSliderProps) {
  ...
  return (
    <div 
      className="bg-gray-800/50 border border-gray-600 p-2 lg:p-2 max-w-xs lg:max-w-sm mx-auto"
      onMouseEnter={() => setIsHovered(true)}
      onMouseLeave={() => setIsHovered(false)}
    >
      <div className="flex items-center justify-between mb-2 lg:mb-2">
        <h3 className="text-xs lg:text-xs font-bold text-topuria-white">
          HISTORIAL UFC
        </h3>
        ...
        <div className="flex items-center gap-1">
          <button
            onClick={prevSlide}
            className="w-6 h-6 lg:w-6 lg:h-6 flex items-center justify-center bg-gray-700 hover:bg-topuria-red transition-colors text-white rounded-sm"
            disabled={currentIndex === 0}
          >
            <ChevronLeft className="w-3 h-3 lg:w-3 lg:h-3" />
          </button>
          ...
          >
            <ChevronRight className="w-3 h-3 lg:w-3 lg:h-3" />
          </button>
        </div>
      </div>

      {/* Fight Cards */}
      <div className="grid grid-cols-1 md:grid-cols-3 gap-1 mb-2 lg:mb-2">
        {visiblePeleas.map((pelea) => (
          <motion.button
            ...
            whileTap={{ scale: 0.98 }}
            className={cn(
              'p-1.5 lg:p-1.5 border transition-all duration-300 relative min-w-0 w-full flex items-center gap-1.5 lg:gap-1.5',
          ...
          >
            {/* Opponent Face */}
            <div className="flex-shrink-0 w-8 h-8 lg:w-10 lg:h-10 border border-white bg-gray-900 overflow-hidden relative">
              <Image
            ...
            <div className="flex-1 min-w-0 text-left">
              <div className="text-xs lg:text-xs font-bold mb-0.5 lg:mb-0.5 truncate">
                {pelea.evento.replace('UFC ', '').replace('UFC on ', '')}
              </div>
              <div className="text-xs lg:text-xs mb-0.5 lg:mb-0.5 truncate">
                vs {pelea.oponente.split(' ').pop()}
              </div>
              <div className="text-xs lg:text-xs text-topuria-gold font-bold">
      ...
      <div className="flex justify-center gap-1 lg:gap-1">
        {Array.from({ length: totalSlides }).map((_, index) => (
          <button
            ...
            className={cn(
              'w-1.5 h-1.5 lg:w-1.5 lg:h-1.5 transition-colors rounded-full',
              currentSlide === index 
                ? 'bg-topuria-red' 
